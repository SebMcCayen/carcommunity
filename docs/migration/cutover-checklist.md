...
- [ ] Firebase Emulator tests pass for all Cloud Functions (unit tests via `npm run test -w @carcommunity/functions` or `vitest run`, emulator tests via the `emulators:test` script in the functions workspace — the exact command depends on the package manager standardized in Phase 3 of the migration plan).
