...
| C8  | **RESOLVED** — subscription expiry now revokes entitlement. The scheduled `subscription-expireLapsed` (`functions/src/subscription/scheduled.ts`, every 3 h) reads `expiresAt` and revokes all three representations (record, `users.activeMember`, `activeMember` claim) via the same `applyEntitlement`, after a 72 h grace window, recording each revocation on `userLifecycle/{uid}.subscriptionExpiry` and notifying the member in-app. A perpetual manual grant (no `expiresAt`) is deliberately never swept. | Part 13.4        |
...
- [x] Subscription expiry correctly revokes `activeMember` claim. Implemented by the scheduled
      `subscription-expireLapsed` sweep (`functions/src/subscription/scheduled.ts`, every 3 h,
      `europe-west1`): a bounded, oldest-lapse-first query over `subscriptions` where the status
      still grants access and `expiresAt` is more than `SUBSCRIPTION_EXPIRY_GRACE_HOURS` (72 h) old,
      revoking through the same `applyEntitlement` as the manual path so all three representations
      (record, `users.activeMember`, `activeMember` claim + refresh-token revocation) are cleared
      with fail-safe ordering. Each revocation is recorded on
      `userLifecycle/{uid}.subscriptionExpiry` and the member gets an in-app `subscription_status`
      notice (no email — deferred project-wide). A PERPETUAL manual grant (no `expiresAt`) is
      deliberately never swept, so the sweep can never revoke on the absence of evidence. Requires
      the `subscriptions` (status, expiresAt) composite index.
