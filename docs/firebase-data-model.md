...
> **Note — make/model are SELECTED, not typed (2026-07):** members pick manufacturer, model and year from dropdowns backed by the static catalogue `contracts/vehicles/vehicle-catalogue.json`, so the community can count cars per manufacturer. `makeId`/`modelId` are the countable keys; `make`/`model` stay as the display text so every existing reader (garage list, public car profile, the `liveLocation` main-car mirror, the map marker subtitle, the admin web) kept working unchanged. A dropdown is UX only — every id is validated server-side against the catalogue in `functions/src/garage/vehicle-catalogue.ts`, because a hand-rolled callable request can send any string it likes.
>
> **Migration stance — nothing is destroyed and nothing is guessed.** Vehicles created before this change keep their free-text `make`/`model` exactly as their owner typed them and simply hold no ids; clients render the stored text whenever `makeId` is absent, so no car changed or disappeared. There is **no backfill**: old free text is never fuzzy-matched onto catalogue ids, because a wrong match ("Wolvo" → `volvo`) is worse than an unmapped car and would silently corrupt the very counts the feature exists for. A member is prompted to select a manufacturer/model only when they next EDIT that vehicle (the form opens with nothing selected and shows the stored legacy value), and an edit never loses data: an "Other / not listed" selection keeps the existing label instead of replacing it with a placeholder. Aggregates must therefore always report the unmapped set (`makeId == null`) alongside the counts rather than pretending it is zero.
