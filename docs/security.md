...
- Store/display latest location only with short TTL: treat location as stale and purge latest-location records/caches after 15 minutes.
...
- Require low speed/stationary condition for claims: 10 km/h for at least 30 seconds is the minimum baseline before claim acceptance; backend configuration may only enforce stricter thresholds.
