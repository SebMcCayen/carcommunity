...
This document describes **both** what exists in the codebase today **and** the proposed extension. Where the two differ, the difference is called out explicitly. Nothing here is implemented purely from this document — it is the shared reference the implementation slices are written against.
...
A crown may only be collected when the device is **not moving**: sustained speed ≤ **2.0 m/s** (7.2 km/h — walking pace) plus a short dwell (see §4.6). The user must have stopped safely before they can reach for the phone.
...
The failure message already exists and is correct: *"Du rör dig för snabbt för att samla in. Stanna säkert innan du samlar in belöningen."*
...
- Sources: `badge`, `event`, `garage`, `admin_adjustment`, `system`, `crown_hunt` (plus a deprecated `future_crown_hunt` kept only for legacy rows).
- Idempotency key **is** the entry document ID, so a replayed award is a transactional no-op that returns the original entry.
...
**The economy in §5 requires no new ledger primitives.** It only needs new callers and new deterministic idempotency keys. That is a good sign about the design of the existing code.
...
| `Δt_u` | Time since user `u` was last present in cell `c` |
...
| Cap | Value | Window |
|---|---|---|
| Global earn cap | **300 KP** | rolling UTC day |
| **Driving-derived** cap `D` | **400 KP** | rolling ISO week |
| Crown claims | 10 | UTC day (already enforced) |
...
Every award carries a **deterministic key derived only from server-trusted values** — never from client input. The key **is** the ledger entry document ID, so replay is a transactional no-op (`functions/src/points/ledger.ts`).

Key shapes, by earn type:

| Earn | Derived from |
|---|---|
| `daily_open` | uid + UTC day |
| `live_session_1km` / `drive_5km` | uid + drive/session ID + km bucket index |
| `crown_collect` | scoped claim key → `claimLedgerIdempotencyKey()` (**already implemented**) |
...
`events-core.ts` permits events with `latitude`/`longitude` null (online meets, or a host who has not pinned the location). For those events, GPS verification is impossible and the system **falls back to the existing conservative proxy already in `badge-core.ts`**: a `going` RSVP on an event that reaches `completed`.
...
Tiered badges use a consistent four-rung ladder — **Brons → Silver → Guld → Platina** — with a KP milestone bonus of **25 / 75 / 200 / 500**, credited via ledger source `badge` and exempt from the daily cap (§5.3).
...
*On C3:* these are lifetime milestones, not per-kilometre payouts. The KP rate for driving is set entirely by the capped `drive_5km`/`live_session_1km` awards in §5.1, so once the weekly budget is spent, further kilometres advance the badge counter but pay **zero KP**. The badge is a record of where you have been, not a meter that keeps ticking. Whether even that is too much encouragement is **Q3**.
...
| # | Stage | Rule | Failure |
|---|---|---|---|
| 1 | Auth + App Check | Signed in, App Check enforced | reject |
| 2 | Account state | Suspended/deleted earn nothing | `not_eligible` |
| 3 | Schema | Strict parse; non-finite/negative speed **rejected** | `invalid-argument` |
| 4 | Idempotency replay | Scoped key already seen → replay stored result | replay |
| 5 | Freshness | Position age ≤ **60 s** | `position_too_old`, +35 risk |
| 6 | Server-side distance | Haversine, server-computed | client distance never read |
| 7 | Geofence | `d ≤ radius + 0.5 · accuracy` | `outside_geofence` |
| 8 | Stationary gate | Speed ≤ **2.0 m/s** sustained (crowns) | `moving_too_fast` |
| 9 | Plausible jump | Implied speed vs last trusted position ≤ **130 m/s** | +40 risk |
| 10 | Accuracy | ≤ **50 m** | +10 risk |
| 11 | Attempt rate | ≥ 4 attempts/min | +25 risk |
| 12 | Success velocity | ≥ 5 awards / 5 min | +15 risk |
| 13 | Fence-edge probing | ≥ 3 edge attempts/hour | +20 risk |
| 14 | Device integrity | Play Integrity / App Attest / mock-location flag | +40 risk |
| 15 | **Risk threshold** | Score ≥ **60** → `risk_review`, **zero KP** | recorded, not awarded |
| 16 | Caps | Enforced **inside** the award transaction | `daily_limit_reached` |
| 17 | Ledger | Append-only, idempotency key = entry ID | replay-safe |
...
- A rooted or jailbroken device running a system-level mock-location provider that feeds a *plausible, smooth, correctly-timed* track. Every signal above is satisfiable by a good simulator. Play Integrity and App Attest raise the cost to "a device you are willing to burn", which is a real deterrent at our scale but not a wall.
...
From Tuesday afternoon onward, **Björn earns nothing further from driving that week.** He can still earn from the Saturday meet (50), hosting (75), his streak (up to 9/day), and confirmed reports (15/day) — roughly 200 KP of genuinely non-driving progress still available to him.
...
| Crown claims/day | 10 |
| Global earn cap | 300 KP/day |
| Driving-derived cap `D` | 400 KP/week |
