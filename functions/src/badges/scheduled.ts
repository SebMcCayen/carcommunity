 ...
 * three cases a trigger cannot cover:
 *
 ...
 * `__name__` needs no composite index and — unlike ordering by a timestamp
 * field — cannot silently skip documents that lack the field. Evaluation of a
 * member with nothing new costs two reads and no writes, so a full pass over
 * an unchanged member base is cheap.
 *
...
import { evaluateAndAwardBadgeTiers } from './tierAwards';
...
export async function runBadgeBacklogSweep(
  ...
  let query = db.collection('badgeProgress').orderBy('__name__').limit(limit);
  if (cursor) {
