...
import { COMMUNITY_CHANNEL_ID, parseAdminDeleteCommunityMessageInput } from './chat-core';
...
export interface AdminDeleteCommunityMessageResponse {
  ...
  deleted: boolean;
  /** How many open reports this call moved to 'reviewed'. */
...
export const adminDeleteMessage = onCall(
  ...
  async (request): Promise<AdminDeleteCommunityMessageResponse> => {
    ...
    return db.runTransaction(async (tx) => {
      ...
      // Every report that escalated THIS community message. Two equality filters
      // (surface + targetId), no range and no orderBy, so Firestore serves it by
      // merging single-field indexes — NO composite index is required. `targetId`
      // is the reported messageId (moderation-core buildMessageReportDocument);
      // `surface` pins it to the community channel so a same-id message on
      // another surface can never be swept in.
      const reportsSnap = await tx.get(
        reportsRef.where('surface', '==', 'community').where('targetId', '==', messageId),
      );
      const openReports = reportsSnap.docs.filter(
        (doc) => doc.data().status === OPEN_REPORT_STATUS,
      );

      // Resolve open reports regardless of whether the message still exists, so
      // the queue never strands a pending report over an already-gone message.
      for (const report of openReports) {
        tx.update(report.ref, { status: RESOLVED_REPORT_STATUS });
      }

      if (!messageSnap.exists) {
        // Idempotent re-delete: nothing to remove, no audit record (nothing
        // happened to the channel), just report resolution above.
        return { messageId, deleted: false, resolvedReports: openReports.length };
      }
        ...
        buildAdminAuditEvent(
          {
            ...
            details: {
              ...
              resolvedReports: openReports.length,
            },
          },
          serverTimestamp,
        ),
      ...
      return { messageId, deleted: true, resolvedReports: openReports.length };
    });
  },
);
