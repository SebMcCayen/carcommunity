...
export const spawnDiagnostics = onCall(
  ...
  async (request): Promise<SpawnDiagnosticsResponse> => {
    ...
    const areaPoiCount = typeof areaData.poiCount === 'number' ? areaData.poiCount : 0;
    ...
    // first), the same ordering `runCrownAreaSpawnPass` reads.
    // -----------------------------------------------------------------------
    ...
    for (const doc of activeAreasSnap.docs) {
      ...
      if (ms < thisPassMs) areasAhead += 1;
    }

    // -----------------------------------------------------------------------
    // Cell enumeration + scan window (from the cursor), mirroring the pass.
    // -----------------------------------------------------------------------
    const enumeration = shape
      ? cellKeysForBoundingBox(shapeBoundingBox(shape), MAX_AREA_CELLS)
    ...
    const cells: SpawnDiagnosticCell[] = await Promise.all(
      scannedKeys.map(async (cellKey): Promise<SpawnDiagnosticCell> => {
    ...
    return {
      ...
      shape: shape as CrownSpawnAreaShape,
      flagEnabled,
      active,
      safeAreaConfirmed,
      areaPoiCount,
