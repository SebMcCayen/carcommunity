...
export const buyPerk = onCall(CALLABLE_OPTS, async (request): Promise<BuyPerkResponse> => {
  ...
  try {
    result = await debitPoints(
      ...
      // Read-phase guard: the per-perk / total-value hold cap, atomic with the debit.
      async (tx) => {
        // Affordability FIRST, so an insufficient-funds rejection always WINS over
        // a hold-cap one. If the member cannot afford the buy, return without a
        // limit throw and let the ledger's own overdraft guard reject it (mapped
        // to `insufficient_funds`) — otherwise a broke member at their hold-cap
        // would get the wrong reason. This also skips the inventory read on what
        // is really an overdraft.
    ...
    // The read-guard's hold-cap rejection (and the ledger's suspended-account
    // guard) are EXPECTED `failed-precondition`s, not infra
